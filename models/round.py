#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Modèle Round - Architecture MVC épurée
Responsabilité : Données et logique métier essentielle uniquement
"""

from typing import List, Optional, Tuple
from .match import Match
from utils.date_utils import get_current_timestamp, calculate_duration_minutes


class Round:

    
    def __init__(self, name: str):

        if not name or not name.strip():
            raise ValueError("Le nom du tour ne peut pas être vide")
            
        self.name = name.strip()
        self.start_time = get_current_timestamp()
        self.end_time: Optional[str] = None
        self.matches: List[Match] = []
        self.is_finished = False
        
    def add_match(self, match: Match):

        if self.is_finished:
            raise ValueError("Impossible d'ajouter un match à un tour terminé")
        if not isinstance(match, Match):
            raise TypeError("L'objet doit être une instance de Match")
        self.matches.append(match)
        
    def remove_match(self, match: Match) -> bool:
        """Supprime un match du tour"""
        if self.is_finished:
            raise ValueError("Impossible de supprimer un match d'un tour terminé")
        try:
            self.matches.remove(match)
            return True
        except ValueError:
            return False
            
    def end_round(self):

        if not self.all_matches_finished():
            unfinished_count = len(self.get_unfinished_matches())
            raise ValueError(f"Il reste {unfinished_count} match(s) non terminé(s)")
        self.end_time = get_current_timestamp()
        self.is_finished = True
        
    def all_matches_finished(self) -> bool:
        """Vérifie si tous les matchs sont terminés"""
        return all(match.is_finished for match in self.matches)
        
    def get_finished_matches(self) -> List[Match]:
        """Retourne les matchs terminés"""
        return [match for match in self.matches if match.is_finished]
        
    def get_unfinished_matches(self) -> List[Match]:
        """Retourne les matchs non terminés"""
        return [match for match in self.matches if not match.is_finished]
        
    def get_completion_percentage(self) -> float:
        """Calcule le pourcentage de completion"""
        if not self.matches:
            return 0.0
        finished_count = len(self.get_finished_matches())
        return (finished_count / len(self.matches)) * 100
        
    def get_duration_minutes(self) -> Optional[int]:
        """Calcule la durée en minutes via les utilitaires"""
        if not self.end_time:
            return None
        return calculate_duration_minutes(self.start_time, self.end_time)
            
    def get_matches_as_tuples(self) -> List[Tuple]:
        """Retourne les matchs sous forme de tuples (conforme spécifications)"""
        return [match.get_match_tuple() for match in self.matches]
        
    def get_match_by_players(self, player1_id: str, player2_id: str) -> Optional[Match]:
        """Trouve un match par les IDs des joueurs"""
        for match in self.matches:
            if ((match.player1_national_id == player1_id and match.player2_national_id == player2_id) or
                (match.player1_national_id == player2_id and match.player2_national_id == player1_id)):
                return match
        return None
        
    def get_matches_for_player(self, player_id: str) -> List[Match]:
        """Retourne tous les matchs d'un joueur dans ce tour"""
        return [match for match in self.matches if match.involves_player(player_id)]
        
    # Méthode déléguée aux helpers (appelée par les contrôleurs)
    def get_round_statistics(self) -> dict:
        """
        Calcule les statistiques du tour
        Logique simple conservée ici car pas trop complexe
        """
        total_matches = len(self.matches)
        finished_matches = len(self.get_finished_matches())
        
        # Calculer les résultats
        wins = sum(1 for match in self.get_finished_matches() if match.get_winner_id() is not None)
        draws = sum(1 for match in self.get_finished_matches() if match.is_draw())
        
        return {
            'name': self.name,
            'total_matches': total_matches,
            'finished_matches': finished_matches,
            'completion_rate': self.get_completion_percentage(),
            'wins': wins,
            'draws': draws,
            'duration_minutes': self.get_duration_minutes(),
            'is_finished': self.is_finished
        }
        
    def to_dict(self) -> dict:
        """Sérialisation simple"""
        return {
            "name": self.name,
            "start_time": self.start_time,
            "end_time": self.end_time,
            "matches": [match.to_dict() for match in self.matches],
            "is_finished": self.is_finished
        }
        
    @staticmethod
    def from_dict(data: dict) -> 'Round':
        """Désérialisation simple"""
        if "name" not in data:
            raise KeyError("Champ requis manquant: name")
            
        round_obj = Round(data["name"])
        round_obj.start_time = data.get("start_time", get_current_timestamp())
        round_obj.end_time = data.get("end_time")
        round_obj.is_finished = data.get("is_finished", False)
        
        # Reconstruire les matchs
        for match_data in data.get("matches", []):
            try:
                match = Match.from_dict(match_data)
                round_obj.matches.append(match)
            except Exception as e:
                print(f"Erreur lors du chargement d'un match: {e}")
                
        return round_obj
        
    def __str__(self) -> str:
        """Représentation textuelle"""
        status = "Terminé" if self.is_finished else "En cours"
        return f"{self.name} - {len(self.matches)} matchs ({status})"
        
    def __repr__(self) -> str:
        """Représentation pour le debug"""
        return f"Round('{self.name}', matches={len(self.matches)}, finished={self.is_finished})"